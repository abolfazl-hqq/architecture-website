...
const Auth = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!isLogin && formData.password !== formData.confirmPassword) {
            setFieldErrors({confirmPassword: 'رمز عبور و تکرار آن مطابقت ندارند'});
            setLoading(false);
            return;
        }
        ...
        try {
            if (isLogin) {
                ...
                login(data.access, {username: formData.username, email: formData.email || ''});
                navigate('/', {replace: true});
            } else {
                // حالت ثبت نام
                const registerData = await authService.register(formData.username, formData.email, formData.password, formData.confirmPassword);
                console.log('Register response:', registerData);

                // بعد از ثبت نام موفق، لاگین خودکار انجام بده
                const loginData = await authService.login(formData.username, formData.password);

                if (!loginData.access) {
                    throw new Error('توکن دریافت نشد');
                }
                login(loginData.access, {username: formData.username, email: formData.email || ''});
                navigate('/', {replace: true});
            }
        } catch (err) {
            console.error('Auth error:', err);

            // خطاهای فیلد (برای ثبت‌نام)
            if (err.response?.data && typeof err.response.data === 'object') {
                // اگر خطا مربوط به فیلد خاصی است (username, email, password)
                if (err.response.data.username || err.response.data.email || err.response.data.password || err.response.data.confirmPassword) {
                    setFieldErrors(err.response.data);
                } else {
                    // خطای عمومی (مثل لاگین ناموفق)
                    const errorMsg = err.response.data.detail || err.response.data.message || 'خطایی رخ داده است';
                    setError(errorMsg);
                }
            }
            // خطای معمولی
                ...
                <div className="relative px-4 py-10 bg-white shadow-lg rounded-3xl sm:p-20">
                    ...
                    <div className="relative mt-6 max-w-md mx-auto text-end">
                            ...
                            <div className="py-8 text-base leading-6 space-y-4 text-muted-700 sm:text-lg sm:leading-7">
                                {!isLogin && (
                                    <div className="relative">
                                        <input
                                            ...
                                            className={`peer placeholder-transparent h-10 w-full border-b-2 ${fieldErrors.email ? 'border-red-500' : 'border-surface-300'} text-background-900 focus:outline-none focus:border-primary-500`}
                                ...
                                <div className="relative">
                                    <input
                                        ...
                                        onChange={handleChange}
                                        className={`peer placeholder-transparent h-10 w-full border-b-2 ${fieldErrors.username ? 'border-red-500' : 'border-surface-300'} text-background-900 focus:outline-none focus:border-primary-500`}
                                ...
                                <div className="relative">
                                    <input
                                        ...
                                        onChange={handleChange}
                                        className={`peer placeholder-transparent h-10 w-full border-b-2 ${fieldErrors.password ? 'border-red-500' : 'border-surface-300'} text-background-900 focus:outline-none focus:border-primary-500`}
                                ...
                                {!isLogin && (
                                    <div className="relative">
                                        <input
                                            ...
                                            onChange={handleChange}
                                            className={`peer placeholder-transparent h-10 w-full border-b-2 ${fieldErrors.confirmPassword ? 'border-red-500' : 'border-surface-300'} text-background-900 focus:outline-none focus:border-primary-500`}
                        ...
                        <div className="w-full flex justify-center mt-6">
                            <button
                                className="flex items-center bg-white border border-surface-300 rounded-lg shadow-md px-6 py-2 text-sm font-medium text-background-800 hover:bg-surface-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500">
                                <svg className="h-6 w-6 ml-2" xmlns="http://www.w3.org/2000/svg"
